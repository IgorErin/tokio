...
use crate::task::{IdProvider, JoinHandle};
...
cfg_rt! {
        ...
        F::Output: Send + 'static,
    {
        ...
        if fut_size > BOX_FUTURE_THRESHOLD {
            spawn_inner(Box::pin(future), SpawnMeta::new_unnamed(fut_size), id)
        } else {
            spawn_inner(future, SpawnMeta::new_unnamed(fut_size), id)
        }
    }

    #[track_caller]
    pub(super) fn spawn_inner<T>(future: T, meta: SpawnMeta<'_>, id: task::Id) -> JoinHandle<T::Output>
    where
